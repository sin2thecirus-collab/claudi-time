...
class CSVImportService:
    ...
    async def process_csv_content(
        ...
        try:
            ...
            # Post-Import Pipeline: Ergebnisse werden in errors_detail["pipeline"] gespeichert
            if successful > 0:
                pipeline = {}
                ...
                try:
                    ...
                    pipeline["categorization"] = {"status": "failed", "error": str(cat_err)[:200]}
                ...
                try:
                    ...
                    pipeline["geocoding"] = {"status": "failed", "error": str(geo_err)[:200]}
                ...
                try:
                    ...
                    pipeline["profiling"] = {"status": "failed", "error": str(profile_err)[:200]}
                ...
                try:
                    ...
                    pipeline["embedding"] = {"status": "failed", "error": str(emb_err)[:200]}
                    logger.warning(f"Auto-Embedding fehlgeschlagen: {emb_err}")

                # 5. Auto-Matching (kostet $0, nutzt Pre-Embeddings)
                try:
                    ...
                    pipeline["matching"] = {"status": "failed", "error": str(match_err)[:200]}
                    logger.warning(f"Auto-Matching fehlgeschlagen: {match_err}")

                # Pipeline-Ergebnisse in DB speichern
                if not import_job.errors_detail:
                    import_job.errors_detail = {}
