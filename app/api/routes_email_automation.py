...
# ══════════════════════════════════════════════════════════════════
# TEMP: Instantly Source Fix (Sonstige → StepStone)
# Kann nach einmaliger Ausfuehrung geloescht werden
# ══════════════════════════════════════════════════════════════════

@router.post("/instantly/fix-source")
async def fix_instantly_source():
    """TEMP: Alle Leads in Instantly-Kampagnen mit source='Sonstige' auf 'StepStone' setzen.

    Liest INSTANTLY_API_KEY aus Railway Environment Variables.
    Iteriert ueber beide Kampagnen und patcht custom_variables.source.
    """
    import os
    ...
    if not api_key:
        return JSONResponse(
            status_code=500,
            content={"error": "INSTANTLY_API_KEY nicht in Environment Variables gesetzt"},
        )

    campaigns = {
        "erstkontakt": "cb3ef455-3578-47aa-80eb-14da499a832c",
        "bekannt": "0074f7e3-dadf-434f-9fbb-5f7d10c1782f",
    }

    results = {"total_checked": 0, "total_fixed": 0, "errors": [], "campaigns": {}}

    async with httpx.AsyncClient(timeout=60.0) as client:
        headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}

        for camp_name, camp_id in campaigns.items():
            camp_results = {"checked": 0, "fixed": 0, "errors": []}
            starting_after = None

            # Paginiert alle Leads laden
            while True:
                ...
                try:
                    ...
                    if resp.status_code != 200:
                        camp_results["errors"].append(f"GET leads: HTTP {resp.status_code} - {resp.text[:200]}")
                        break

                    data = resp.json()
                    leads = data if isinstance(data, list) else data.get("items", data.get("data", []))

                    if not leads:
                        break

                    for lead in leads:
                        camp_results["checked"] += 1
                        cv = lead.get("custom_variables", {})
                        source_val = cv.get("source", "")

                        if not source_val or source_val.lower() == "sonstige":
                            # Lead updaten
                            ...
                            try:
                                patch_resp = await client.patch(
                                    ...
                                    json={
                                        "campaign_id": camp_id,
                                        "custom_variables": {"source": "StepStone"},
                                    },
                                )
                                if patch_resp.status_code < 400:
                                    camp_results["fixed"] += 1
                                else:
                                    camp_results["errors"].append(
                                        f"PATCH {email}: HTTP {patch_resp.status_code}"
                                    )
                            except Exception as e:
                                camp_results["errors"].append(f"PATCH {email}: {str(e)[:100]}")

                    # Pagination
                    if len(leads) < 100:
                        break
                    starting_after = leads[-1].get("id", leads[-1].get("email"))

                except Exception as e:
                    camp_results["errors"].append(f"GET leads error: {str(e)[:200]}")
                    break

            results["total_checked"] += camp_results["checked"]
            results["total_fixed"] += camp_results["fixed"]
            results["errors"].extend(camp_results["errors"])
            results["campaigns"][camp_name] = camp_results

    logger.info(f"Instantly Source Fix: {results['total_checked']} geprueft, {results['total_fixed']} gefixt")
    return results
