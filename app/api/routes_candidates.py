...
async def drive_config_check():
    """Debug: Zeigt alle GOOGLE/DRIVE env vars."""
    import os
    google_vars = {
        k: f"{v[:8]}...({len(v)}ch)"
        for k, v in os.environ.items()
        if "GOOGLE" in k or "DRIVE" in k
    }
    return {
        "env_vars_found": google_vars,
        "pydantic_client_id_len": len(settings.google_drive_client_id),
        "pydantic_secret_len": len(settings.google_drive_client_secret),
        "pydantic_token_len": len(settings.google_drive_refresh_token),
        "pydantic_folder_len": len(settings.google_drive_folder_id),
    }
...
async def parse_cv_for_quickadd(
    ...
    async with CVParserService(db) as parser:
        ...
        try:
            ...
            pass  # Kein Text → Vision-Fallback unten

        # Parsen: Text oder Vision-Fallback
        if cv_text and len(cv_text.strip()) >= 50:
            # Standard: Text-basiertes Parsing
            parse_result = await parser.parse_cv_text(cv_text)
        else:
            # Vision-Fallback (Bild-PDFs, Canva, Print-to-PDF etc.)
            logger.info(f"Quick-Add Vision-Fallback (kein extrahierbarer Text im PDF)")
            try:
                ...
                return {"success": False, "message": f"PDF konnte weder als Text noch als Bild verarbeitet werden: {e}"}
